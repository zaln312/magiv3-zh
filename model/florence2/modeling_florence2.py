...
class Florence2ForConditionalGeneration(Florence2PreTrainedModel):
    ...
    def predict_detections_and_associations(
        ...
        for (
            ...
            categories = [
                map_to_category.get(generated_text[j : j + 3], None)
    ...
    def predict_character_grounding(
        ...
        batch_inputs = processor(
            batch_input_text=[
                convert_caption_to_instruction(caption) for caption in captions
            ],
            batch_input_list_of_list_of_bboxes=[[]] * len(images),
            batch_images=images,
            padding=True,
        ...
        return [
            ...
            for generated_text, list_of_list_of_bboxes, indices_of_bboxes_in_generated_text in zip(
                generated_texts,
                list_of_list_of_list_of_bboxes,
                batch_indices_of_bboxes_in_generated_text,
            )
    ...
    def forward(
        ...
        character_character_affinity_matrices = (
            self.get_character_character_affinity_matrices(
        ...
        text_character_association_matrices = (
            self.get_text_character_association_matrices(
                outputs.decoder_hidden_states[-1],
                decoder_input_ids,
                tokenizer,
                apply_sigmoid=True,
    ...
    def slowly_generate_grounded_caption(
        ...
        **kwargs,
        ...
        decoder_input_ids = processor.tokenizer(
            captions, return_tensors="pt", truncation=False, padding=True
        ...
        while True:
            ...
            what_should_be_the_next_tokens = torch.stack(
                [
                    decoder_input_ids[
                        i, running_indices[i] + 1 : running_indices[i] + CHUNK_SIZE + 1
                    ]
            ...
            if predicted_next_tokens.shape[1] == what_should_be_the_next_tokens.shape[
                1
            ] and torch.all(predicted_next_tokens == what_should_be_the_next_tokens):
            ...
            for i, (pnts, pnt) in enumerate(
                zip(predicted_next_token_strings, predicted_next_tokens)
            ):
                if (
                    pnts.startswith("<loc_") or pnts in ["<s>", "<pad>", "</s>"]
                ) and running_indices[i] < decoder_input_ids.shape[1] - 1:
    ...
    def get_character_character_affinity_matrices(
        self, decoder_hidden_states, decoder_input_ids, tokenizer, apply_sigmoid=False
    ):
        character_character_affinity_matrices = []
        for index in range(len(decoder_hidden_states)):
            character_embedding_indices = (
                (
                    decoder_input_ids[index]
                    == tokenizer.convert_tokens_to_ids("<character>")
                )
                .nonzero()
                .squeeze(-1)
            )
            character_embeddings = decoder_hidden_states[index][
                character_embedding_indices
            ]
    ...
    def get_text_character_association_matrices(
        self, decoder_hidden_states, decoder_input_ids, tokenizer, apply_sigmoid=False
    ):
        text_character_association_matrices = []
        for index in range(len(decoder_hidden_states)):
            text_embedding_indices = (
                (decoder_input_ids[index] == tokenizer.convert_tokens_to_ids("<text>"))
                .nonzero()
                .squeeze(-1)
            )
            ...
            if character_embeddings.shape[0] == 0 or text_embeddings.shape[0] == 0:
                text_character_association_matrices.append(
                    torch.zeros(
                        text_embeddings.shape[0], character_embeddings.shape[0]
                    ).type_as(text_embeddings)
                )
                continue
            text_i = repeat(
                text_embeddings,
    ...
    def get_text_tail_association_matrices(
        self, decoder_hidden_states, decoder_input_ids, tokenizer, apply_sigmoid=False
    ):
        ...
        for index in range(len(decoder_hidden_states)):
            ...
            tail_embedding_indices = (
                (decoder_input_ids[index] == tokenizer.convert_tokens_to_ids("<tail>"))
                .nonzero()
                .squeeze(-1)
            )
            ...
            if tail_embeddings.shape[0] == 0 or text_embeddings.shape[0] == 0:
                text_tail_association_matrices.append(
                    torch.zeros(
                        text_embeddings.shape[0], tail_embeddings.shape[0]
                    ).type_as(text_embeddings)
                )
                continue
            text_i = repeat(
                text_embeddings, "i d -> i repeat d", repeat=tail_embeddings.shape[0]
            )
            tail_j = repeat(
                tail_embeddings, "j d -> repeat j d", repeat=text_embeddings.shape[0]
            )
    ...
    def get_tail_character_association_matrices(
        self, decoder_hidden_states, decoder_input_ids, tokenizer, apply_sigmoid=False
    ):
        ...
        for index in range(len(decoder_hidden_states)):
            ...
            if character_embeddings.shape[0] == 0 or tail_embeddings.shape[0] == 0:
                tail_character_association_matrices.append(
                    torch.zeros(
                        tail_embeddings.shape[0], character_embeddings.shape[0]
                    ).type_as(tail_embeddings)
                )
                continue
            tail_i = repeat(
                tail_embeddings,
